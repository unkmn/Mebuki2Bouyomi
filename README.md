...
# ダウンロード<!-- omit in toc -->
[＞＞＞ Mebuki2Bouyomi ダウンロード ＜＜＜](https://github.com/unkmn/Mebuki2Bouyomi/releases/download/v.1.0.0/mebuki2bouyomi_1.0.0.zip)<br>
<br>
- 2025/11/06 ver.1.0.0
  - 画面右下に簡易操作パネルを追加（表示無しにも変更可）

<br>

<details><summary>過去の変更履歴（クリックで展開）</summary>
- 2025/11/05 ver.beta2 (0.2.2) 
  - 「新着レスの通知」機能を追加
  - 設定画面を閉じると設定欄のタブが初期化される問題の修正
  - めぶきのページ遷移に対応
  - 棒読みちゃん・わんコメとの連携に失敗した場合にエラーメッセージを表示するように改修
  - 複数の新着レスを一度に取得した時の読み上げ精度を再度改修
  - UIを少し整理
- 2025/11/03 ver.beta(0.2.1) 
  - インラインブロックおよびコードブロックを読み上げないように改修
  - わんコメ連携時、複数の新着レスを一度に取得した時の読み上げ精度を改修
  - 各設定タブから当ドキュメントの説明欄へ飛べるリンクを追加
- 2025/11/03 ver.beta(0.2.0)
  - 公開
...
# 目次<!-- omit in toc -->
- [主な機能](#主な機能)
  - [画面右下に簡易操作パネルを表示する](#画面右下に簡易操作パネルを表示する)
  - [スレッド閲覧支援機能](#スレッド閲覧支援機能)
    - [新着レスを通知する](#新着レスを通知する)
    - [新着レスのファイルを保存する](#新着レスのファイルを保存する)
    - [スレッド内のファイルを一括DLする](#スレッド内のファイルを一括dlする)
  - [棒読みちゃん連携機能](#棒読みちゃん連携機能)
...
# 主な機能
## 画面右下に簡易操作パネルを表示する
画面の右下、レスアイコンの上あたりに簡易的な操作パネルを表示します。<br>
画面の横幅が狭いと邪魔になると思うので、好みで表示ON/OFFしてください。

<br>

## スレッド閲覧支援機能
### 新着レスを通知する
新着レスを検知した場合、OSの通知でお知らせします。<br>

### 新着レスのファイルを保存する
新着レスに画像ファイルが添付されていた場合、自動ダウンロードします。<br>
...
以下の機能を利用する場合は **「配信支援機能を利用する」をONにすることで設定可能**になります。<br>
スレタイやスレ本文に含まれたら自動で有効にするキーワードを設定できます。
